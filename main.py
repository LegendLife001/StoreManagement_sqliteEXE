...
try:
    conn= sql.connect(r".\database\store.db")
except sql.OperationalError:
    os.makedirs("./database")
    conn= sql.connect(r".\database\store.db")
cur= conn.cursor()
TableName="inventory"
cur.execute(f"CREATE TABLE if not exists inventory (ID INTEGER PRIMARY KEY AUTOINCREMENT, Product_Name VARCHAR(100) NOT NULL, stock INT NOT NULL, cp INT NOT NULL, sp INT NOT NULL, totalcp INT, totalsp INT, assumed_profit INT, Vender VARCHAR(100), date Varchar(12), time Varchar(12));")
        ...
        def get_items(self, *args, **kwargs):
            ...
            if self.stock.isdigit() and self.cp.isdigit() and self.sp.isdigit(): 
                ...
                if self.name.lower() not in namess:
                    ...
                    self.clear()
                    self.name_e.focus()
                else:       # if product already exists then we can just increace the no. of stocks
...
def stock_modify(*args, **kwargs):     #stock modification section to modify data in the db
    ...
    class Stock_modify:                  #running Stock_Modify class when stock_modify button is called
        def __init__(self, root,*args, **kwargs):
            ...
            val=[]
            cur.execute("select id, Product_Name from inventory order by id")
            ...
            def callback(selection, *args, **kwargs):
                ...
                if selection=="By Date":
                    ...
                    def btt_fun(*args, **kwargs):
                        ...
                        cur.execute(f"select * from {TableName} WHERE date='{date}'")
                        data=cur.fetchall()
                        self.count=0
                        if data!=[]:
                            for i in data:
                    ...
                    def callme(*args, **kwargs):
                        ...
                        try:
                            ...
                            return
                        self.trees.delete(*self.trees.get_children())
                        cur.execute(f"select * from {TableName} where date like '%.{m}.{s.get()}'")
                        data = cur.fetchall()
                        self.count=0
                    ...
                    def callme(*args, **kwargs):
                        ...
                        sel=s.get()
                        self.trees.delete(*self.trees.get_children())
                        cur.execute(f"select * from {TableName} where date like '%{sel}'")
                        data = cur.fetchall()                        
                        if data!=[] and data!=None:
                            for i in data:
                                self.trees.insert(parent='', index='end', iid=self.count, text="", values=(f"{i[0]}",f"{i[1]}",f"{i[2]}", f"{i[3]}", f"{i[4]}", f"{i[5]}", f"{i[6]}", f"{i[7]}", f"{i[8]}", f"{i[9]}", f"{i[10]}"))
                    ...
                    ll1.lift()
                    cur.execute(f"select Vender from {TableName}")
                    val=[]
                    for x in cur:
                        if x[0] not in val:
                            val.append(x[0])
                    def callme(*args, **kwargs):
                        ...
                        sel= me.get().lower()
                        self.trees.delete(*self.trees.get_children())
                        cur.execute(f"select * from {TableName} WHERE Vender='{sel}'")
                        data=cur.fetchall()
                        self.count=0
                        if data!=[]:
                            for i in data:
...
================================================\n''')
            ...
            def fun_sortdate(*args, **kwargs):
                ...
                try:
                    ...
                    val_d, val_m, val_y= IntVar(), IntVar(), IntVar()
                    datee=["0"+str(i) for i in range(1, 10)] + [str(i) for i in range(10, 32)]
                    d= Spinbox(master, value= datee, textvariable=val_d, width=5, font=("Ariel", 15, "bold"), state="readonly")
                    d.place(x=190, y=30)
                    monthh= ["0"+str(i) for i in range(1, 10)]+['10', '11', '12']
                    m= Spinbox(master, value= monthh, textvariable=val_m, width=5, font=("Ariel", 15, "bold"), state="readonly")
                    m.place(x=290, y=30)
                    y= Spinbox(master, from_=2021, to_=3000, textvariable=val_y, width=5, font=("Ariel", 15, "bold"), state="readonly")
                    y.place(x=390, y=30)
                if ch:
                    def calp(*args, **kwargs):
                        cal.place(x=30, y=10)
                ...
                def btt_fun(*args, **kwargs):
                    ...
                    try:
                        ...
                        date= get_date()
                    self.trees.delete(*self.trees.get_children())
                    cur.execute(f"select * from transactions WHERE date='{date}'")
                    data=cur.fetchall()
                    self.count=0
                    if data!=[]:
                        for i in data:
            ...
            def fun_sortmonth(*args, **kwargs):
                ...
                def callme(*args, **kwargs):
                    ...
                    cur.execute(f"select * from transactions where date like '%.{m}.{s.get()}'")
                    data = cur.fetchall()
                    self.count=0
            ...
            def fun_sortyear(*args, **kwargs):
                ...
                def callme(*args, **kwargs):
                    ...
                    sel=s.get()
                    self.trees.delete(*self.trees.get_children())
                    cur.execute(f"select * from transactions where date like '%{sel}'")
                    data = cur.fetchall()
                    self.count=0
                    if data!=[] and data!=None:
                        for i in data:
...
class Billing:
    def __init__(self,root1,left, right, *args, **kwargs):
        # getting all tables
        c= cur
        c.execute("Create table if not exists transactions(ID INTEGER PRIMARY KEY AUTOINCREMENT, Product_Name VARCHAR(100),Quantity INT,Received_Amount INT, bill_profit INT, date VARCHAR(12), time VARCHAR(12), invoice INT(10));")
        conn.commit()
        ...
        if len(prodquan)==0:
            ...
            def submit_fun(*args, **kwargs):
                ...
                try:
                    ...
                    randomid= random.randint(1111, 99999)
